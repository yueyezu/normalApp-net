        ...
        public static string TreeViewJson(this List<TreeViewModel> data, string parentId = "0")
        {
            ...
            if (item.Count > 0)
            {
                foreach (TreeViewModel entity in item)
                {
                    ...
                    if (entity.iconCls != null && !string.IsNullOrEmpty(entity.iconCls.Replace("&nbsp;", "")))
                    {
                        strJson.Append("\"iconCls\":\"" + entity.iconCls.Replace("&nbsp;", "") + "\",");
                    }
                    if (entity.checkstate != null && !entity.hasChildren)
                    {
                    ...
                    strJson.Append("\"children\":" + TreeViewJson(data, entity.id) + "");
                    strJson.Append("},");
                }
