...
* Desp: 用于后台进行网络请求时调用的方法。在进行集群环境内的协调时会比较实用
*/
...
namespace LT.Code
{
    public class RequestHelper
    {
        ...
        public static string HttpPost(string url, string param = null)
        {
            ...
            request.ContentType = "application/x-www-form-urlencoded";
            request.Accept = "*/*";
            request.Timeout = 1000 * 30;    //毫秒 30秒
            request.AllowAutoRedirect = false;
        ...
        /// <returns></returns>
        public static string HttpPostWithPic(string url, IDictionary<object, object> param, string filePath)
        {
        ...
        /// <returns></returns>
        public static string HttpPostWithPic(string url, IDictionary<object, object> param, byte[] fileByte)
        {
            string boundary = "---------------------------" + DateTime.Now.Ticks.ToString("x");
            byte[] boundarybytes = System.Text.Encoding.ASCII.GetBytes("\r\n--" + boundary + "\r\n");
